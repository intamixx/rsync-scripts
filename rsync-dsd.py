...
"""
Usage: ./rsync-dsd.py [stop|stop|status|restart]
"""
...
class MyDaemon(Daemon):
        def run(self):
                ...
                host = ''
                port = ''
                remote_dsd_dir = '/export/dsd-tmp1'
                local_dsd_dir = '/export/dsd-tmp1'
                ...
                while True:
                        ...
                        result = os.path.exists('%s' % local_dsd_dir )
                        if result == False:
                                logit("Directory %s not Found" % local_dsd_dir )
                                try:
                                        os.makedirs('%s' % local_dsd_dir )
                                except OSError:
                                        logit("Could not create directory %s" % local_dsd_dir )
                                        logit('ERROR: Could not create directory %s' % local_dsd_dir )
                        ...
                        for host in nameservers:
                                ...
                                logit("Launching dsd %s -> %s" % (rsync, host) )
                                runStr = "%s -4 -avz  --include=\"*/\" --include=\"*.js\" --exclude=\"*\" -e \"ssh -4 -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null\" --delete --progress %s:%s/ %s" % (rsync, host, remote_dsd_dir, local_dsd_dir)

                                ( retCode, stdout, stderr ) = run(host,runStr,shell=True)

                                retCode = int(retCode)
                                if (retCode == 0):
                                        print "%s\n%s" % (stdout, stderr)
                                        logit("Dsd Rsync Command exited normally %s: %s" % (host, retCode) )
                                else:
                                        print "%s\n%s" % (stdout, stderr)
                                        logit("Dsd Rsync Command failed %s: %s" % (host, retCode) )
...
def run(host, args, cwd = None, shell = False, kill_tree = True, timeout = -1, env = None):
    ...
    logit("Running dsd %s %s -> Pid [%d]" % (prog, host, p.pid) )
    if ( timeout != -1 ):
    ...
    try:
        ...
        for pid in pids:
            ...
            logit("Killing dsd %s %s -> Pid [%d] " % (prog, host, p.pid) )
            try:
                kill(pid, SIGKILL)
            except OSError:
                pass
...
def get_process_children(pid):
    p=subprocess.Popen('ps --no-headers -o pid --ppid %d' % pid,shell=True,stdout=PIPE,stderr=PIPE)
    stdout, stderr = p.communicate()
...
if __name__ == "__main__":
        daemon = MyDaemon('/tmp/rsync-dsd.pid')
        if len(sys.argv) == 2:
                if 'start' == sys.argv[1]:
                        daemon.start()
